...
void Database::updPMStatus(int msgId, int status)
{
    m_query = "UPDATE " + mysql_table_PM + " SET status = " + QString::number(status) + " WHERE id = " + QString::number(msgId);
    sendRequest("UPDATE PRIVATE MSG status");
}

void Database::updAMStatus(const QString& msgId, const QString& forUser, const QString& status)
{
