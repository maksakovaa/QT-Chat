...
void Net::requestHandler(QTcpSocket* cliSocket)
{
    ...
    else if (request.startsWith("CHG_PWD"))
    {
        chgPwd();
    }
...
void Net::regUser(QTcpSocket* cliSocket)
{
    ...
    {
        sendPersonalRequest("REG_USER<|>ERROR: " + tr("user login already exists"), cliSocket);
    }
    else
    {
        SQL_DataBase->regUser(newUser);
        SQL_DataBase->getUsrBase();
        sendPersonalRequest("REG_USER<|>" + name + "<|>" + UserBase->getUUID(login), cliSocket);
    }
