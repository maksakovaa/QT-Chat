...
QString SHA256::operator()(const void* data, size_t numBytes)
{
    reset();
    add(data, numBytes);
    return getHash();
...
QString SHA256::operator()(const QString& text)
{
    reset();
    add(text.toStdString().c_str(), text.size());
    return getHash();
}
